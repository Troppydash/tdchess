...
/// SEARCH PARAMETERS ///
TUNE(HISTORY_MULT, 329, 0, 100, 400);
TUNE(HISTORY_BASE, -237, 0, -400, -100);
TUNE(MAX_HISTORY_UPDATE, 1450, 0, 1000, 3000);

TUNE(HISTORY_MALUS_MULT, 301, 0, 100, 400);
TUNE(HISTORY_MALUS_BASE, -243, 0, -400, -100);

TUNE(SNM_MARGIN, 67, 0, 25, 150);

TUNE(NMP_DEPTH, 0, 1, 0, 10);
TUNE(NMP_REDUCTION_MULT, 4, 1, 1, 10);
TUNE(NMP_REDUCTION_BASE, 7, 1, 1, 10);

TUNE(QUIET_LMR_DIV, 10000, 1, 1000, 15000);
TUNE(CAPTURE_LMR_DIV, 6000, 1, 1000, 15000);
...
TUNE(RAZOR_DEPTH_MULT, 300, 1, 200, 400);
...
TUNE(QUIET_BAD_THRESHOLD, -10000, 1, -7000, -3000);
